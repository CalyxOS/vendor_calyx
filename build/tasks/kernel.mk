...
## Internal variables
COLLECT_MODULE_DEPS_CMD := python3 $(BUILD_TOP)/calyx/scripts/collect-kernel-module-deps/collect-kernel-module-deps.py --non-interactive
...
ifeq ($(BOARD_USES_QCOM_MERGE_DTBS_SCRIPT),true)
	...
	PATH=$(abspath $(HOST_OUT_EXECUTABLES)):$${PATH} python3 $(BUILD_TOP)/vendor/calyx/build/tools/merge_dtbs.py --base $(DTBS_BASE) --techpack $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor --out $(DTBS_OUT)
...
$(TARGET_PREBUILT_INT_KERNEL): $(DEPMOD) $(KERNEL_MODULES_PARTITION_FILE_LIST) $(SYSTEM_KERNEL_MODULES_PARTITION_FILE_LIST)
	...
	$(hide) cd $(BUILD_TOP) && \
		python3 vendor/calyx/build/tools/kernel_kleaf_manifest.py $(TARGET_KERNEL_VERSION) \
