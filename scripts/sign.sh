...
declare -gA device_to_preexisting_build=()
declare -gA preexisting_build_to_devices=()
...
prompt_missing_variables() {
  ...
  local deduced
  deduced=$(deduce_build_number) || return $?
  ask_variable BUILD_NUMBER "$deduced" n y || return $?
  deduced=$(deduce_prev_build_number) || return $?
  ask_variable PREV_BUILD_NUMBER "$deduced" n n || return $?
...
main() {
  ...
  # Again, as now we want to limit it to PREV_BUILD_NUMBER.
  try find_prev_signed_device_builds || return $?
...
sign_builds() {
  ...
  export TMPDIR=$(mktemp -d -p "${ORIGINAL_TMPDIR:-/dev/shm}" sign_tmp.XXXXXXXX)
  # We already make sure keys are available in advance, so no need to try and load per process.
  export ENSURE_KEY_IS_AVAILABLE=n
...
initialize() {
  export TMPDIR=$(mktemp -d -p "$ORIGINAL_TMPDIR" signing_tmp.XXXXXXXX)
  export STATEDIR=$(mktemp -d -p /dev/shm signing_state.XXXXXXXX)
  echo
  if ! is_vendor_initialization_needed; then
    ...
    try maybe_initialize_vendor || return $?
  fi
...
#    if { [ -z "$prev_build" ] || [ "$prev_build" -lt "$build" ]; } && \
#       { [ -z "$next_build" ] || [ "$build" -gt "$next_build" ]; }; then
...
maybe_handle_already_existing_builds() {
  ...
  for buildnum in $BUILD_NUMBER; do
    if [ -n "${preexisting_build_to_devices[$buildnum]:-}" ]; then
  ...
  if [ "$any_already_signed" = "y" ]; then
    ...
    for buildnum in $BUILD_NUMBER; do
      local devices_to_show=${preexisting_build_to_devices[$buildnum]:-}
      [ -n "$devices_to_show" ] || continue
      printf " %s: %s\n" "$buildnum" "$devices_to_show"
    done
    echo
    echo "If you continue, these device/build pairs will not be signed again."
...
find_prev_signed_device_builds() {
  ...
  for dir in "${prev_signed_dirs[@]}"; do
    ...
    if [ -n "${PREV_BUILD_NUMBER:-}" ]; then
      local matches=
      local buildnum
      for buildnum in $PREV_BUILD_NUMBER; do
        if [ "$build" = "$buildnum" ]; then
          matches=y
          break
        fi
    ...
    if [ -f "archive/$dir/$device-target_files-$build.zip" ]; then
      local prev_build=${device_to_prev_build[$device]:-}
      local next_build=${device_to_next_build[$device]:-}
      if [ -z "$prev_build" ] || [ "$build" -gt "$prev_build" ] && \
         { [ -z "$next_build" ] || [ "$build" -lt "$next_build" ] ; }; then
        device_to_prev_build[$device]=$build
      fi
    fi
  done
  prev_build_to_devices=()
  local device
...
deduce_prev_build_number() {
  # Again, so that it factors in our chosen BUILD_NUMBER.
