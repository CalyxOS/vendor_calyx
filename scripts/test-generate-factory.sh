...
VERSION=$(cat build/make/core/build_id.mk | grep "BUILD_ID=" | cut -d = -f 2 | tr '[:upper:]' '[:lower:]')
...
if [[ $DEVICE == marlin || $DEVICE == sailfish || $DEVICE == taimen || $DEVICE == walleye ||
  ...
  BOOTLOADER=$(cat vendor/google/${DEVICE}/android-info.txt | grep version-bootloader | cut -d = -f 2)
  BOOTLOADERSRC=bootloader-${DEVICE}-${BOOTLOADER,,}.img
  RADIO=$(cat vendor/google/${DEVICE}/android-info.txt | grep version-baseband | cut -d = -f 2)
