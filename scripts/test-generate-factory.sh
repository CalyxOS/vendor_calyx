...
if [[ $DEVICE == marlin || $DEVICE == sailfish || $DEVICE == taimen || $DEVICE == walleye ||
  ...
  BOOTLOADER=$(cat $ANDROID_BUILD_TOP/vendor/google/${DEVICE}/android-info.txt | grep version-bootloader | cut -d = -f 2)
  BOOTLOADERSRC=bootloader-${DEVICE}-${BOOTLOADER,,}.img
  RADIO=$(cat $ANDROID_BUILD_TOP/vendor/google/${DEVICE}/android-info.txt | grep version-baseband | cut -d = -f 2)
