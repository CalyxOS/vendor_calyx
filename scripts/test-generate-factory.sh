#!/bin/bash
# Script to test factory image scripts
...
VERSION=$(cat $ANDROID_BUILD_TOP/build/make/core/build_id.mk | grep "BUILD_ID=" | cut -d = -f 2 | tr '[:upper:]' '[:lower:]')
...
if [[ $DEVICE == marlin || $DEVICE == sailfish || $DEVICE == taimen || $DEVICE == walleye ||
  ...
  BOOTLOADER=$(cat $ANDROID_BUILD_TOP/vendor/google/${DEVICE}/factory/android-info.txt | grep version-bootloader | cut -d = -f 2)
  RADIO=$(cat $ANDROID_BUILD_TOP/vendor/google/${DEVICE}/factory/android-info.txt | grep version-baseband | cut -d = -f 2)
...
echo "NOT Creating signed targetfiles zip"
...
#else
#echo "Don't Create OTA update zip"
...
echo "NOT Creating factory images"
...
unzip $DEVICE-$VERSION-factory-*.zip
mv $DEVICE-$VERSION/* .
rmdir $DEVICE-$VERSION
rm $DEVICE-$VERSION-factory-*.zip
#sha256sum $DEVICE-factory-$BUILD.zip | awk '{printf $1}' > $DEVICE-factory-$BUILD.zip.sha256sum

#popd

echo "NOT Removing intermediate file after factory image generation: $DEVICE-img-$BUILD.zip"
