...
_we_initialized_vendor_release_child=
...
maybe_initialize_vendor_release_child() {
  if is_vendor_initialization_needed && \
    ...
    _we_initialized_vendor_release_child=y
  fi
}
...
common_cleanup_with_vendor() {
  ...
  if [ "$_we_initialized_vendor_release_child" ]; then
    finalize_vendor_release_child || true
  fi
  common_cleanup || true
}
