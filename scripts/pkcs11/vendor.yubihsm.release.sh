...
if [ -n "${YUBIHSM_LOGS_DIR:-}" ]; then
  ...
  DEVICE=$1
  export AUDIT_LOG_PATH=${AUDIT_LOG_PATH:-$YUBIHSM_LOGS_DIR/audit-$timestamp-$BUILD_NUMBER-$DEVICE.log}
  export BUILD_LOG_PATH=${BUILD_LOG_PATH:-$YUBIHSM_LOGS_DIR/build-$timestamp-$BUILD_NUMBER-$DEVICE.log}
fi

if [ -n "${BUILD_LOG_PATH:-}" ]; then
  "$scriptpath/release.sh" "$@" 2>&1 | tee -a "$BUILD_LOG_PATH" || exit $?
else
