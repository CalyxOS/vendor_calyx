...
initialize_pre_metadata() {
  ...
  declare -g unique_key_out_dir=${UNIQUE_KEY_OUT_DIR:-$key_out_dir/$(get_name_for_hsm_and_session)}
  declare -g hsm_key_out_dir=${HSM_KEY_OUT_DIR:-$key_out_dir/$yubihsm_id}
}
...
does_key_exist_yn() {
  local exists
  exists=$(does_key_exist_yn_pkcs11 "$@") || return $?
...
does_cert_exist_yn() {
  local exists
  exists=$(does_cert_exist_yn_pkcs11 "$@") || return $?

  if [ "$exists" = "n" ]; then
    # Ensure it does not exist on disk, in which case we don't want to recreate it.
    local cert_id=$1
    local cert_file=$key_out_dir/$key_id.x509.pem
    if [ -e "$cert_file" ]; then
      echo y
    else
      echo n
