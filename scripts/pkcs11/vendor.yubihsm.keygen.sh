...
generate_cert() {
  ...
  yubihsm -a put-opaque \
    ...
    --domain "$domain" \
    --capabilities "$capabilities" \
    --in "$unique_key_out_dir/$key_id.x509.pem" \
    || return $?

  # Output the certificate and the public key.
  case "$key_type" in
    avb|apex_payload|apex_container)
      if [ -e "$unique_key_out_dir/$key_id.pem" ]; then
        echo "Public key file already exists: $unique_key_out_dir/$key_id.pem" >&2
      else
        # Output public key and avbpubkey for APEX payload or AVB keys.
        # Also do this for core keys, since APEX might use them.
        out=$unique_key_out_dir/$key_id.pem
        "$OPENSSL_BIN" x509 -pubkey -noout -in "$unique_key_out_dir/$key_id.x509.pem" \
          -out "$out" \
          || return $?
        copy_exported_file_to_other_dirs "$out" || return $?
      fi
      if [ -e "$unique_key_out_dir/$key_id.avbpubkey" ]; then
        echo "avbpubkey file already exists: $unique_key_out_dir/$key_id.avbpubkey" >&2
      else
        out=$unique_key_out_dir/$key_id.avbpubkey
        "$AVBTOOL_BIN" extract_public_key \
          --key "$unique_key_out_dir/$key_id.pem" \
          --output "$out" \
          || return $?
        copy_exported_file_to_other_dirs "$out" || return $?
      fi
