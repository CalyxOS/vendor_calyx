...
initialize_pre_metadata() {
  ...
  source "$SCRIPTPATH/vendor.yubihsm.include.sh" || exit $?
...
generate_cert() {
  ...
  case "$key_type" in
    avb|apex_payload|apex_container)
      ...
      if [ "$key_type" = "avb" ] && [ "$key_name" = "vbmeta" ]; then
        # Copy AVB key to avb_custom_key.img.
        if [ -e "$dstdir/avb_custom_key.img" ]; then
          echo "avb_custom_key file already exists: $dstdir/avb_custom_key.img" >&2
        else
          cp "$dstdir/$name.avbpubkey" "$dstdir/avb_custom_key.img"
        fi
      fi
      ;;
  esac
