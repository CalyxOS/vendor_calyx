...
declare -ga delta_pairs_with_loaded_keys=()
declare -ga delta_pairs_without_loaded_keys=()
...
do_or_show() {
  ...
  if [ "$no_sign" != "y" ] && [ "${#devices_with_loaded_keys[@]}" -gt 0 ]; then
    $maybe_dry_run sign_builds "${devices_with_loaded_keys[@]}" || return $?
  fi

  if [ "$no_delta" != "y" ] && [ ${#delta_pairs_with_loaded_keys[@]} -gt 0 ]; then
    $maybe_dry_run generate_deltas "${delta_pairs_with_loaded_keys[@]}" || return $?
  fi
  ...
  if [ "$no_sign" != "y" ] && [ "${#devices_without_loaded_keys[@]}" -gt 0 ]; then
    $maybe_dry_run sign_builds "${devices_without_loaded_keys[@]}" || return $?
  fi

  if [ "$no_delta" != "y" ] && [ ${#delta_pairs_without_loaded_keys[@]} -gt 0 ]; then
    $maybe_dry_run generate_deltas "${delta_pairs_without_loaded_keys[@]}" || return $?
  fi
}
...
find_prev_signed_device_builds() {
  ...
  for device in "${devices_to_sign[@]}"; do
  ...
  if [ "${#missing_prior_builds[@]}" -gt 0 ]; then
    echo
    echo "Found no prior builds for devices we plan to sign:"
...
sort_and_separate_devices_by_key_availability() {
  # next, remove any devices whose keys are not loaded into the HSM
  # finally, add those devices onto the end
  local device
  local -A devices_to_sign_map=()

  # 1. Sort devices by their order in all_devices
  for device in "${devices_to_sign[@]}"; do
    devices_to_sign_map[$device]=1
  done
  devices_to_sign=()
  delta_pairs=()
  for device in "${all_devices[@]}"; do
    if [ -n "${devices_to_sign_map[$device]:-}" ]; then
      devices_to_sign+=("$device")
    fi
  done
  ...
  local err=
  populate_devices_with_loaded_keys_arrays "${devices_to_sign[@]}" || err=$?
  if [ -n "$err" ]; then
    {
  ...
  for device in "${devices_with_loaded_keys[@]}"; do
    local build=${device_to_prev_build[$device]:-}
    [ -n "$build" ] || continue
    delta_pairs_with_loaded_keys+=("$device,$build")
  done
  for device in "${devices_without_loaded_keys[@]}"; do
    local build=${device_to_prev_build[$device]:-}
    [ -n "$build" ] || continue
    delta_pairs_without_loaded_keys+=("$device,$build")
  done
}
