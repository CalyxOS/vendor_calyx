...
prompt_missing_variables() {
  ...
  ask_variable NUM_SIGN_JOBS "3" n y
  ask_variable NUM_DELTA_JOBS "3" n y
}
...
sign_builds() {
  ...
  parallel -j "$NUM_SIGN_JOBS" --tag --line-buffer \
    echo "$pkcs11_scriptpath/vendor.yubihsm.release.sh" "{}" \
      "calyx_{}-target_files-${BUILD_NUMBER}.zip" \
    "&& echo" mv "out/calyx_{}-target_files-${BUILD_NUMBER}" archive/. \
    ::: "${devices_to_sign[@]}" || err=$?
...
generate_deltas() {
  ...
  parallel -j "$NUM_DELTA_JOBS" --match '(.*),(.*)' --tag --line-buffer \
    echo "$pkcs11_scriptpath/vendor.yubihsm.generate_delta.sh" "{1.1}" "{1.2}" ${BUILD_NUMBER} \
    ::: "${delta_pairs[@]}" || err=$?
