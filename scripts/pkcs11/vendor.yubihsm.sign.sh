...
parallel -j 3 --tag --line-buffer ./vendor/calyx/scripts/pkcs11/vendor.yubihsm.release.sh {} calyx_{}-target_files-${BUILD_NUMBER}.zip ::: akita shiba bluejay FP5 hawao &pid=$!
...
parallel -j 3 --tag --line-buffer ./vendor/calyx/scripts/pkcs11/vendor.yubihsm.generate_delta.sh {} ${PREV_BUILD_NUMBER_PXL} ${BUILD_NUMBER} ::: akita shiba bluejay &pid=$!

wait $pid

echo "Generating delta updates for remaining devices"

parallel -j 3 --tag --line-buffer ./vendor/calyx/scripts/pkcs11/vendor.yubihsm.generate_delta.sh {} ${PREV_BUILD_NUMBER_MOST} ${BUILD_NUMBER} ::: FP5 hawao &pid=$!
