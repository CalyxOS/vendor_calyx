...
delete_temporary_authentication_key_primary() {
  ...
  local err=
  local args=(-a delete-object --object-id "$YUBIHSM_TEMPORARY_AUTHKEY_ID" \
    --object-type authentication-key)

  YUBIHSM_AUTHKEY=$YUBIHSM_TEMPORARY_AUTHKEY_ID YUBIHSM_PASSWORD=password \
  yubihsm_nolog "${args[@]}" || err=$?

  # We cannot use the yubihsm function's built-in logging because it will try to use the
  # newly-deleted authkey to extract logs. So, we just do it on our own.
  extract_logs "" "Command: yubihsm-shell$(printf ' %q' "${args[@]}")"$'\n'"Result: ${err:-0}" \
    || return $?
}
...
log_provisioning_start() {
  ...
  local objects
  objects=$(yubihsm_nolog -a list-objects) || return $?
  local manifest
...
reset_hsm() {
  local err=
  yubihsm_nolog -a list-objects || err=$?
  echo
...
EOF
  ...
  if [ -n "$err" ]; then
    echo "Due to the error above, you likely need to reset the HSM manually by removing and" >&2
