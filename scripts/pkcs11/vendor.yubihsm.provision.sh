...
main() {
  ...
  # Both overriden in log_provisioning_start.
  export YUBIHSM_LOGS_DIR=$KEEP_PATH/logs
  export AUDIT_LOG_PATH=preprovisioning-$(date -u +%s.%3N).log
...
copy_files() {
  ...
  local manifest_file=$SOURCE_DIRECTORY/$shipped_manifest_relpath
  local -a manifest_files
  mapfile -t manifest_files < <(cat "$manifest_file" | cut -d$'\t' -f 1 | grep -Fxv filename) || return $?
  local file
  ...
  cp -pr "$manifest_file" "$PROVISIONING_PATH/$shipped_manifest_relpath" || return $?
}

verify_copied_files() {
  local manifest_file=$basepath/$shipped_manifest_relpath
  verify_files "$manifest_file" "$PROVISIONING_PATH" || return $?
}
...
connect_hsm_primary() {
  ...
  log_provisioning_start primary || return $?
}
...
provision_wrap_key_primary() {
  ...
  echo
  confirm "Are you ready to input this information, followed by shares?" || return $?
...
log_provisioning_end_primary() {
  log_provisioning_end primary || return $?
}
...
connect_hsm_secondary() {
  ...
  log_provisioning_start secondary || return $?
}
...
restore_secondary() {
  ...
  local file
  for file in "$KEEP_PATH/keys/"*.yhw; do
    if [ ! -e "$file" ]; then
      echo "No wrapped keys found. Please place keys (.yhw files) in: $KEEP_PATH/keys" >&2
  ...
  echo
  echo "Finished restoring wrap key with yubuhsm-setup. Now trying to restore all available"
  echo "keys to ensure that works, making room as needed with ondemand keys..."
  KEY_DIR="$KEEP_PATH/keys" try restore_all_keys_and_certs "$@" || return $?
...
log_provisioning_end_secondary() {
  log_provisioning_end secondary || return $?
}
...
log_provisioning_start() {
  ...
  export YUBIHSM_LOGS_DIR=$KEEP_PATH/logs
  export AUDIT_LOG_PATH=provision-$(get_name_for_hsm_and_session).log
  ...
  local manifest
  manifest=$(cat "$basepath/$shipped_manifest_relpath") || return $?
  add_input_to_log <<EOF || return $?
Begin provisioning mode $mode for $1
$info
-Manifest-
$manifest
EOF
...
reset_hsm_manual() {
  local info=${1:-}
  echo >&2
  echo "Either the HSM could not be found, or we could not communicate with it." >&2
  echo "If the HSM is connected, you likely need to reset it manually by removing and" >&2
  echo "re-inserting it, pressing on the metal rim for at least 10 seconds." >&2
  echo >&2
  echo "If you'd like to do so, please do so now." >&2
  echo "Press Y if you have manually reset the device and want to reset it again," >&2
  echo "to be certain that this step was completed, or N if you don't want to reset." >&2
  echo >&2
...
log_provisioning_end() {
  ...
  add_input_to_log <<EOF || return $?
End provisioning mode $mode for $1
$(show_yubihsm_info)
EOF
}
...
verify_files() {
  ...
  mapfile -t manifest_lines < "$manifest_file" || return $?
  local line
  local header_done=
  ...
  while true; do
    ...
    case "$response" in
      ...
      S|s)
        ...
        break ;;
      *)
        echo "Please enter Y, N, or S to skip." >&2 ;;
    esac
    break
  done
}
