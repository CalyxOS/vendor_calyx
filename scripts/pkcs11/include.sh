...
_get_privkey_uri() {
  local key=$1
  if [ "${FIXME_I_TRULY_LOVE_WHEN_THINGS_DO_NOT_WORK:-n}" = "y" ] && [ "${FIND_KEYS_BY_ID:-n}" = "y" ]; then
    uri="pkcs11:id=%$(_escape_for_uri "$key")"
  else
