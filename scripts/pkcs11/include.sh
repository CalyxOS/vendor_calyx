...
if [ -z "$OPENSSL_BIN" ] && [ -z "$PKCS11_TOOL_BIN" ]; then
  echo "Please install openssl or set OPENSSL_BIN to its path." >&2
  exit 1
fi
if [ -z "$PKCS11_TOOL_BIN" ]; then
  echo "pkcs11-tool not found. Please install opensc or set PKCS11_TOOL_BIN to its path." >&2
  echo "Attempting to proceed without it." >&2
fi

find_openssl_engine_library() {
  if [ -z "$OPENSSL_BIN" ] || [ -n "${PKCS11_OPENSSL_ENGINE_LIBRARY_PATH:-}" ]; then
    return 0
  fi
  ...
  local path; for path in "${possible_paths[@]}"; do
    if [ -e "$path" ]; then
      PKCS11_OPENSSL_ENGINE_LIBRARY_PATH=$path
      export PKCS11_OPENSSL_ENGINE_LIBRARY_PATH
      return 0
    fi
  done
  echo "Please install the OpenSSL engine for PKCS#11 modules (libengine-pkcs11-openssl)," >&2
  echo "or set PKCS11_OPENSSL_ENGINE_LIBRARY_PATH to its path." >&2
  return 1
}
find_openssl_engine_library || exit $?
...
does_id_exist_for_type_yn_pkcs11() {
  local id=$1
  local type=$2
  id=$(get_key_id_for_pkcs11-tool "$id")
  local -a args
  ...
  output=$(PKCS11_PIN=${PKCS11_PIN:-} PKCS11_SO_PIN=${PKCS11_SO_PIN:-} \
    "$PKCS11_TOOL_BIN" --list-objects --type "$type" --id "$id" \
    "${args[@]}") || return $?
  if ! printf "%s\n" "$output" | sed -n -e 's/^  ID: \+//p' | grep -qFx -- "$id"; then
    echo n
  else
    echo y
  fi
...
# Function to get the key algorithm from a given key type and name. Overridable.
# Called by generate_keypair_pkcs11 in keygen.include.sh.
# May be called/overridden by vendor-specific scripts or keymappers.
get_key_algorithm() {
  get_default_key_algorithm "$@" || return $?
}

# The default behavior of get_key_algorithm in case overriding scripts want to fall back to it.
get_default_key_algorithm() {
  local key_type=${1:-}
  if [ "$key_type" = "avb" ] || [ "$key_type" = "apex_payload" ]; then
    # Must be RSA.
    echo "RSA:${RSA_KEY_SIZE:-4096}"
  else
    # More flexible, but for now we use RSA here as well.
    echo "RSA:${RSA_KEY_SIZE:-4096}"
  fi
}
