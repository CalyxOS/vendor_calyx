...
get_key_id_is_exportable_yn() {
  echo "ERROR: get_key_id_is_exportable_yn not implemented." >&2
  exit 1
}
...
does_keygen_use_pkcs11_to_generate_key() {
  echo "ERROR: does_keygen_use_pkcs11_to_generate_key not implemented." >&2
  echo "       If you use the built-in PKCS#11 keygen capabilities, implement as 'true'," >&2
  echo "       else 'false'." >&2
  exit 1
}

does_keygen_use_pkcs11_to_generate_cert() {
  echo "ERROR: does_keygen_use_pkcs11_to_generate_cert not implemented." >&2
  echo "       If you use the built-in PKCS#11 cert generation capabilities, implement" >&2
  echo "       as 'true', else 'false'." >&2
  exit 1
}
...
keygen_main() {
  ...
  maybe_initialize_vendor_interactive || return $?
  ...
  if [ "$_any_key_generated" != "y" ]; then
    echo "No keys generated during the process."
  fi
  if [ "$_any_cert_generated" != "y" ]; then
    echo "No certs generated during the process."
  fi
}
...
initialize_post_metadata_common() {
  ...
  if does_keygen_use_pkcs11_to_generate_key; then
  ...
  if does_keygen_use_pkcs11_to_generate_cert; then
