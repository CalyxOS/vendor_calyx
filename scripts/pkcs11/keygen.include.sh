    ...
    for key in "${!indirect_array}"; do
      ...
      for device in "${devices_array[@]}"; do
        ...
        if [ -n "${already_generated[$value]:-}" ]; then
          echo "# VALUE: $value: skipping, generated ${already_generated[$value]}" >&2
          continue
        fi
        if ! DEVICE=$device ensure_key_not_exist "$value"; then
          #if [ "$value" != "$last_already_exist" ]; then
            echo "# VALUE: $value: already exists"
          #fi
          already_generated[$value]="before this run"
          last_already_exist=$value
          continue
        fi
        echo "# VALUE: $value"
        DEVICE=$device _generate_keypair_and_cert "$value" "$key_type" "$key" || return $?
...
load_metadata() {
  ...
  if [ -z "${METADATA_FILE:-}" ]; then
    ...
    [ -e "$METADATA_FILE" ] || METADATA_FILE=$(realpath -e -q "$SCRIPTPATH/../metadata")
...
load_keymapper() {
  ...
  if [ -n "${KEYMAPPER_FILE:-}" ]; then
    keymapper_path=$(cd "$SCRIPTPATH/.."; realpath -e "$KEYMAPPER_PATH") || return $?
    source "$KEYMAPPER_FILE" || return 1
  else
    local keymapper=${KEYMAPPER:-legacy}
    keymapper_path=$(cd "$SCRIPTPATH/.."; realpath -e "keymapper.$keymapper.sh" || true)
...
initialize_pre_metadata_pkcs11() {
  ...
  case "${PKCS11_MODULE:-}" in
    ""|*softhsm*)
      ...
      if [ ! -e "$SOFTHSM2_CONF" ]; then
        ...
        _generate_softhsm_config > "$SOFTHSM2_CONF" || return $?
      fi
  ...
  PKCS11_OPENSSL_ENGINE_LIBRARY_PATH=${PKCS11_OPENSSL_ENGINE_LIBRARY_PATH:-$(find \
    /usr/local -path '/usr/local/lib*/engines-3/libpkcs11.so' | head -n 1)}
  PKCS11_OPENSSL_ENGINE_LIBRARY_PATH=${PKCS11_OPENSSL_ENGINE_LIBRARY_PATH:-$(find \
    /usr -path '/usr/lib*/engines-3/libpkcs11.so' | head -n 1)}
  if [ -z "$PKCS11_OPENSSL_ENGINE_LIBRARY_PATH" ]; then
    echo "Please install openssl pkcs11 engine library, or set PKCS11_OPENSSL_ENGINE_LIBRARY to its path." >&2
    missing_something=1
  fi
  if [ -n "$missing_something" ]; then
    return 1
  fi
  if [ "${MODIFICATION_USES_SO_PIN:-}" = "y" ] && [ -z "${PKCS11_SO_PIN:-}" ]; then
...
generate_cert_pkcs11() {
  ...
  if [ "${KEY_LABEL_IS_HEX:-n}" = "y" ]; then
    key_label=$(get_key_id "$key_type" "$key_label") || return $?
  fi
  ...
  OPENSSL_CONF=<(_generate_openssl_config) \
    "$OPENSSL_BIN" req -new -x509 -engine pkcs11 \
      ...
      -sha256 \
      -keyform engine \
      -key "$(_get_privkey_uri "$key_label")" \
...
ensure_key_not_exist_pkcs11() {
  ...
  local key_label=$1
  result=$(FIND_KEYS_BY_ID=n _get_id_for_key "$key_label") || exit $?
  if [ -n "$result" ]; then
