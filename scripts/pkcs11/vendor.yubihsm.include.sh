...
initialize_vendor() {
  ...
  declare -g YUBIHSM_TMPDIR=
  if [ "${DRY_RUN:-}" != "y" ]; then
    declare -g YUBIHSM_TMPDIR
    YUBIHSM_TMPDIR=$(mktemp -d "${TMPDIR:-/dev/shm}/yhrelease.XXXXXXXX") || return $?
  ...
  export YUBIHSM_LOCKFILE=$YUBIHSM_TMPDIR/yubihsm.lock
}
...
is_yubihsm_connector_running() {
  if pgrep -fx "$YUBIHSM_CONNECTOR_BIN" >/dev/null || pgrep -fx yubihsm-connector >/dev/null; then
    true
  else
    false
  fi
...
initialize_vendor_release() {
  ...
  if [ -n "${YUBIHSM_LOGS_DIR:-}" ]; then
    ...
    default_prefix=$BUILD_NUMBER-$DEVICE-$(get_name_for_hsm_and_session)-$(basename "$command" .sh)
...

  {
    if [ -n "${YUBIHSM_LOCKFILE:-}" ]; then
      ...
      echo "$(date -u) $$ flocking initialize_vendor_release_child" >> /tmp/yubihsm_sign_debug.log
      flock -x 3
      echo "$(date -u) $$ proceeding initialize_vendor_release_child" >> /tmp/yubihsm_sign_debug.log
    fi
...
finalize_vendor_release() {
  ...
  if [ -n "${YUBIHSM_TMPDIR:-}" ] && [ -d "$YUBIHSM_TMPDIR" ]; then
    rm -rf "$YUBIHSM_TMPDIR"
  fi
  common_cleanup || true
}
