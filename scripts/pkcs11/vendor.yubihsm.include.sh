...
YUBIHSM_SIGNING_AUTHKEY_CAPABILITIES=${YUBIHSM_SIGNING_AUTHKEY_CAPABILITIES:-generate-asymmetric-key,sign-pkcs,sign-pss,sign-ecdsa,sign-eddsa,derive-ecdh,import-wrapped,export-wrapped,exportable-under-wrap,get-option,sign-attestation-certificate,get-log-entries,change-authentication-key,decrypt-pkcs,decrypt-oaep,put-opaque,get-opaque,delete-object}
...
limit_ondemand_objects() {
  ...
  ondemand_keys=($(yubihsm_nolog -a list-objects \
...
ensure_key_is_available_internal() {
  ...
  local needed_key
  needed_key=$(yubihsm_nolog -a list-objects \
    --object-id "$key_id" \
    --object-type asymmetric-key \
  ...
  local ondemand_key
  ondemand_key=$(yubihsm_nolog -a list-objects \
    --object-type asymmetric-key \
    --domain "$YUBIHSM_ONDEMAND_DOMAIN" \
    | sed -n -e 's/^id: \(0x[^,]\+\),.*$/\1/p' \
    | head -n1) || return $?
