...
add_input_to_log_raw() {
  local text_file=${1:-${AUDIT_LOG_PATH:-}}
  if [ -z "$text_file" ]; then
    # Nowhere to save logs, so don't.
    return 0
  fi
...
ensure_key_is_available_internal() {
  ...
  if [ ! -e "${KEY_DIR}${key_id}-asymmetric-key.yhw" ]; then
  ...
  if [ ! -e "${KEY_DIR}${ondemand_key}-asymmetric-key.yhw" ]; then
  ...
  #  --in "${KEY_DIR}0x${key_id}-opaque.yhw" \
  #  || return $?
...
dump_object_info() {
  ...
  mapfile -t object_lines < <(yubihsm_nolog -a list-objects) || return $?
