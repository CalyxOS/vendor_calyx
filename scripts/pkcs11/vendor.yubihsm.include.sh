...
extract_logs() {
  ...
  if [ -z "$text_file" ] && [ -z "${AUDIT_LOG_PATH+x}" ]; then
    text_file=audit-$timestamp.log
  fi
  if [ -z "$hex_file" ] && [ -z "${AUDIT_LOG_PATH_HEX+x}" ]; then
    hex_file=${text_file:-audit-$timestamp.log}
    hex_file=${hex_file%.log}.hex
  fi
...
initialize_release_vendor() {
  ...
  local timestamp
  timestamp=$(date -u +%Y-%m-%d-%H-%M-%S)
  export AUDIT_LOG_PATH=${AUDIT_LOG_PATH:-audit-$timestamp-$BUILD_NUMBER-$DEVICE.log}
  PREPEND_LINE="BUILD_NUMBER=$BUILD_NUMBER DEVICE=$DEVICE" \
  APPEND_LINE="---" \
  NO_WARN=y \
    extract_logs \
    || return $?
