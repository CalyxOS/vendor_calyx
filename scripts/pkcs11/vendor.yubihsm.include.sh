...
ensure_key_is_available_no_log() {
  ...
  for key_id in "${key_ids[@]}"; do
    ...
    if [ -z "$key_index" ]; then
      if [ -n "$deletion_candidate_key_id" ]; then
        ...
        debuggy yubihsm -a delete-object --object-id "$deletion_candidate_key_id" \
          --object-type asymmetric-key \
          || return $?
        deletion_ineligible_keys_map[$deletion_candidate_key_id]=1
      fi
      # Import wrapped key.
      debuggy yubihsm -a put-wrapped \
    ...
    if [ -z "$cert_index" ]; then
      if [ -n "$deletion_candidate_cert_id" ]; then
        ...
        debuggy yubihsm -a delete-object --object-id "$deletion_candidate_cert_id" \
          --object-type opaque \
          || return $?
        deletion_ineligible_certs_map[$deletion_candidate_key_id]=1
      fi
      # Import cert. It is not sensitive so really does not need to be wrapped and unwrapped.
      debuggy yubihsm -a put-opaque \
        --object-id "$key_id" \
        --object-type opaque \
...
populate_devices_with_loaded_keys_arrays() {
  ...
  # Get devices from arguments, or if no arguments, get from devices array.
  if [ $# -eq 0 ]; then
    set -- "${devices[@]}"
  fi
...
restore_all_keys_and_certs() {
  ...
  for file in "$KEY_DIR"*.x509.pem "$KEY_DIR"*-asymmetric-key.yhw; do
    ...
    found[$key_id]=1
    for_restore=y ensure_key_is_available_no_log "$key_id" || return $?
  done
}
...
ensure_key_is_available() {
  ...
  log_path=$(get_log_path) || return $?
  local err=0
  ensure_key_is_available_no_log "$@" || err=$?
...
maybe_start_or_restart_yubihsm_connector() {
  ...
  if [ "$start_only" = "y" ] && \
      pgrep -fx "$YUBIHSM_CONNECTOR_BIN" >/dev/null || pgrep -fx yubihsm-connector >/dev/null; then
  ...
  maybe_stop_yubihsm_connector || return $?
  ...
  if [ ! -e "$YUBIHSM_CONNECTOR_PIDFILE" ]; then
    echo "$(date -u) $$ pidfile does not exist somehow" >> /tmp/yubihsm_sign_debug.log
    echo "$(date -u) $$ pidfile does not exist somehow" >&2
  fi
...
maybe_start_or_restart_apksigner_batch() {
  ...
  maybe_stop_apksigner_batch "$APKSIGNER_BATCH_RUNTIME_DIR" \
    "$apksigner_batch_pid" "$apksigner_batch_fifo_keepalive_pid" || return $?
...
maybe_stop_yubihsm_connector() {
  if [ "$_we_started_yubihsm_connector" != "y" ]; then
...
maybe_stop_apksigner_batch() {
  if [ "$_we_started_apksigner" != "y" ]; then
    return 0
  fi
  local runtime_dir=${1:-}
...
initialize_release_vendor() {
  {
    if [ -n "${YUBIHSM_LOCKFILE:-}" ]; then
      ...
      echo "$(date -u) $$ flocking initialize_release_vendor" >> /tmp/yubihsm_sign_debug.log
      flock -x 3
      echo "$(date -u) $$ proceeding initialize_release_vendor" >> /tmp/yubihsm_sign_debug.log
    fi
