...
ensure_key_is_available_no_extract_logs_at_end() {
  ...
  if [ "$is_key_id_ondemand" = "y" ] || \
    ...
    # Do not bother with any of this. It's either not on-demand, or we're not in restore
  ...
  for i in $(seq 0 $((${#object_ids[@]}-1))); do
    ...
    local delete_from_beginning=n  # n = delete from the end of the list.
...
maybe_start_or_restart_yubihsm_connector() {
  ...
  YUBIHSM_CONNECTOR_PIDFILE=${YUBIHSM_CONNECTOR_PIDFILE:-${1:-}}
  ...
  export YUBIHSM_CONNECTOR_PIDFILE
  ...
  echo "$(date -u) $$ we started connector"
}
