...
run_command_maybe_batch() {
  ...
  if [ "$is_a_signing_command" = "y" ]; then
    echo "$(date -u) $$ sourcing include" >> /tmp/yubihsm_sign_debug.log
    source "$pkcs11_scriptpath/vendor.yubihsm.include.sh" || return $?
    echo "$(date -u) $$ sourced" >> /tmp/yubihsm_sign_debug.log
    if [ "$jar" = "apksigner" ] \
      && [ -n "${APKSIGNER_BATCH_RUNTIME_DIR:-}" ]; then
      use_apksigner_batch=y
      echo "$(date -u) $$ maybe_start_apksigner_batch" >> /tmp/yubihsm_sign_debug.log
      maybe_start_apksigner_batch || return $?
      echo "$(date -u) $$ maybe started" >> /tmp/yubihsm_sign_debug.log
    fi
  fi
