...
# ID prefix layout, as defined in associative arrays and variables below.
# 00-0f Reserved
...
# AOSP per-device key starting prefix, as per above. (default 0x20)
...
#         all: Every AVB partition is signed with the vbmeta key. (default)
...
#             The ID used is that of the first APEX container key. (deafult)
