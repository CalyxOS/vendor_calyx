...
ifeq ($(TARGET_KERNEL_LIBCLANG_PATH),)
    ifneq ($(wildcard $(TARGET_KERNEL_SOURCE)/rust/bindings/bindings_helper.h),)
        TARGET_KERNEL_LIBCLANG_PATH := $(shell $(BUILD_TOP)/vendor/calyx/build/tools/select_kernel_libclang.sh \
...
ifneq ($(KERNEL_RBE_WRAPPER),)
    KERNEL_CC_WRAPPER := $(BUILD_TOP)/vendor/calyx/build/tools/kernel_rbe_cc.sh
else
